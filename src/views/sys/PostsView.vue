...
// 编辑表单数据
const editForm = reactive<PostEditForm>({
  id: '',
  name: '',
  code: '',
  sort: 1,
...
const confirmEdit = async (editFormEl: FormInstance | undefined) => {
  ...
  await editFormEl.validate((isValid, invalidFields) => {
    ...
    if (isAdd.value) {
      PostApi.save(editForm, {loadingOption: {target: '.el-dialog'}, showOkMsg: true}).then(response => afterEdit(response))
    } else {
      PostApi.update(editForm, {loadingOption: {target: '.el-dialog'}, showOkMsg: true}).then(response => afterEdit(response))
    }
  })
}
